...
% Antenna parameters
%ant.type = antType;
ant.type = 'helix'; % 'bowtie' 'helix' 'pencil' 'isotropic' 'dipole'
...
for ii = 1:size(txrx,1)
    ...
    switch ant.loc
        ...
        case 'real'
            [ve,fig_ant] = antennaLoc(ant.loc,txrx(ii,:),dPhy,quadrant,off,doPlot);
        case 'virtual' 
            [ve,fig_ant] = antennaLoc(ant.loc,txrx(ii,:),dPhy,doPlot);
    end
...
WComb = [ant.RE; W; WWeight];
[fig,ax,pp] = plotBPPolar(db(WComb(:,901:2701),'power'), thetaScA(:,901:2701), 50);%, txrx); % Unweighted
...
if doSave
    ...
    cd(fileLoc)
    set(fig,'color','w')
    export_fig(fig,strcat(ant.type,'_AF_w_',num2str(thetaStA),'_',...
